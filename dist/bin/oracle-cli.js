...
import { DEFAULT_MODEL, renderPromptMarkdown, readFiles } from '../src/oracle.js';
import { CHATGPT_URL } from '../src/browserMode.js';
import { loadChromeCookies } from '../src/browser/chromeCookies.js';
import { createRemoteBrowserExecutor } from '../src/remote/client.js';
import { applyHelpStyling } from '../src/cli/help.js';
import { collectPaths, collectModelList, parseFloatOption, parseIntOption, parseSearchOption, usesDefaultStatusFilters, resolvePreviewMode, normalizeModelOption, normalizeBaseUrl, resolveApiModel, inferModelFromLabel, parseHeartbeatOption, parseTimeoutOption, } from '../src/cli/options.js';
...
function normalizeRemoteCookieSource(input) {
    if (!input) {
        return undefined;
    }
...
async function populateRemoteCookiesFromLocal(config) {
    const filterNames = Array.isArray(config.cookieNames) && config.cookieNames.length > 0
...
program
    ...
    .option('-m, --model <model>', 'Model to target (gpt-5.1-pro | gpt-5.1 | gpt-5.1-codex, or ChatGPT labels like "5.1 Instant" for browser runs).', normalizeModelOption, DEFAULT_MODEL)
    ...
    .addOption(new Option('--remote-cookie-source <mode>', 'Cookie source for remote runs (local | none).')
    .choices(['local', 'none'])
    .hideHelp())
...
async function runRootCommand(options) {
    ...
    const remoteCookieSource = normalizeRemoteCookieSource(options.remoteCookieSource ?? process.env.ORACLE_REMOTE_COOKIE_SOURCE) ?? 'none';
    if (remoteHost) {
