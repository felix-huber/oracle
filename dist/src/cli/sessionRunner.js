...
export async function performSessionRun({ sessionMeta, runOptions, mode, browserConfig, cwd, log, write, version, notifications, browserDeps, }) {
    ...
    try {
        if (mode === 'browser') {
            ...
            const result = await runBrowserSessionExecution({ runOptions, browserConfig, cwd, log, cliVersion: version }, browserDeps);
            await sessionStore.updateSession(sessionMeta.id, {
                status: 'completed',
